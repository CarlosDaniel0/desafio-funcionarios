...
const EditRole = ({ role }) => {
  ...
  const form = useFormik({
    ...
    onSubmit: async values => {
      await fetch(process.env.NEXT_PUBLIC_API + '/role/' + role.id, {
        method: 'PUT',
        headers: {
    ...
    validate: (values, props) => {
      ...
      if (values.description == '') erros.lastname = 'Digite uma Descrição'
  ...
  useEffect(() => {
    if (role) {
      ...
      form.setFieldValue('wage', role.wage)
